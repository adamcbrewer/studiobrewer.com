...
(function(c, n) {
    ...
    c.fn.imagesLoaded = function(f) {
        ...
        function j(b, a) {
            b.src === l || -1 !== c.inArray(b, k) || (k.push(b), a ? h.push(b) : i.push(b), 
            c.data(b, "imagesLoaded", {
                isBroken: a,
                src: b.src
            }), o && d.notifyWith(c(b), [ a, e, c(i), c(h) ]), e.length === k.length && (setTimeout(m), 
        ...
        e.length ? e.bind("load.imagesLoaded error.imagesLoaded", function(b) {
            ...
            if (e && e.src === d) j(a, e.isBroken); else if (a.complete && a.naturalWidth !== n) j(a, 0 === a.naturalWidth || 0 === a.naturalHeight); else if (a.readyState || a.complete) a.src = l, 
            a.src = d;
        }) : m();
    ...
    */
            render_section: function(template, context, partials) {
                ...
                var regex = this.getCachedRegex("render_section", function(otag, ctag) {
                    ...
                    // all the crap at the beginning that is not {{*}} ($1)
                    // {{
                    //  #foo (# == $2, foo == $3)
                    // }}
                    // between the tag ($2). leading newlines are dropped
                    // {{
                    //  /foo (backreference to the opening tag).
                    // }}
                    // everything else in the string ($4). leading whitespace is dropped.
                    return new RegExp("^([\\s\\S]*?)" + otag + "(\\^|\\#)\\s*(.+)\\s*" + ctag + "\n*([\\s\\S]*?)" + otag + "\\/\\s*\\3\\s*" + ctag + "\\s*([\\s\\S]*)$", "g");
                });
                // for each {{#foo}}{{/foo}} section do...
                return template.replace(regex, function(match, before, type, name, content, after) {
                    // before contains only tags, no sections
                    var renderedBefore = before ? that.render_tags(before, context, partials, true) : "", // after may contain both sections and tags, so use full rendering function
                    renderedAfter = after ? that.render(after, context, partials, true) : "", // will be computed below
...
function Hammer(element, options, undefined) {
    ...
    function getXYfromEvent(event) {
        ...
        if (!_has_touch) {
            if (options.allow_touch_and_mouse && event.touches !== undefined && event.touches.length > 0) {
                _fn = getXYTouch;
            }
        } else {
            _fn = getXYTouch;
...
var Portfolio = function() {
    ...
    var portfolio = {
        ...
        renderIntro: function() {
            var introHTML = ich.ichIntro(this.portfolio), that = this;
        ...
        findProject: function(slug) {
            var i = 0, projects = this.portfolio.projects;
            for (var k = 0; k < this.numProjects; k++) {}
            for (i; i < this.numProjects; i++) {
                if (projects[i].slug === slug) {
                    var project = projects[i], prev = projects[i - 1] || false, next = projects[i + 1] || false;
                    return {
                        current: project,
                        prev: prev,
                        next: next
        ...
        openProject: function(slug) {
            var project = this.findProject(slug), that = this, selectedProjectNum = false;
            if (slug.length === 0) slug = "404";
            this.appendQuery("project", slug);
            this.detailCont.fadeOut(100, function() {
                that.busy();
                var projectHTML = ich.ichProjectDetail({
        ...
        renderGrid: function(show) {
            ...
            var projectsHTML = ich.ichGrid(this.portfolio), that = this;
        ...
        get: function(what, hash) {
            ...
            if (hash && hash.length) {
                this.navLinks.removeClass("current").filter('[data-get="' + what + '"][data-hash]').addClass("current");
            } else {
                this.navLinks.removeClass("current").filter('[data-get="' + what + '"]').not("[data-hash]").addClass("current");
            }
        },
        /**
        ...
        filterProjects: function(tag) {
            ...
            if (typeof tag === "string" && tag != "false") {
                var projectsToShow = this.projectsCont.find('[data-tags*="' + tag + '"]'), projectsToHide = this.projectsCont.find(".grid-project").not(projectsToShow);
                // applying a 'current' state to what we want
                this.tagsCont.find("[data-filter]").removeClass("current").filter('[data-filter="' + tag + '"]').addClass("current");
                // this.busy();
                // this.projectsCont.fadeOut(10, function () {
                projectsToShow.removeClass("hidden");
                //.show(10);
                projectsToHide.addClass("hidden");
                //.hide(10);
                // that.ready(function () {
                ...
                if (tag === false || tag === "false") {
                    ...
                    that.projectsCont.find("[data-tags]").removeClass("hidden");
                    //.show(10);
                    // that.ready(function () {
                    // this.projectsCont.fadeIn(200);
                    // });
        ...
        applyToggle: function(what) {
            ...
            if (what === "tags") {
                ...
                this.togglers.find("[data-toggle]").not('[data-toggle="tags"]').removeClass("current").filter('[data-toggle="' + what + '"]').addClass("current");
                // this.busy();
        ...
        appendQuery: function(key, value) {
            if (Modernizr.history) {
                ...
                var search = window.location.search, q = search.replace("?", "").queryToObj(), k, isInHistory = false;
		 ...
		 */
        parseQuery: function() {
            var search = window.location.search.replace("?", ""), q = search.queryToObj();
            if (q.project) {
                this.openProject(q.project);
            }
            if (q.filter) {
                this.applyToggle("tags").filterProjects(q.filter).get("projects");
            }
            if (q.view) {
                if (q.view === "grid" || q.view === "list") {
                    this.applyToggle(q.view).get("projects");
        ...
        hiddenProjects: function() {
            var projects = this.projectsCont.find(".grid-project"), hiddenProjects = projects.filter(".hidden");
        ...
        visibleProjects: function() {
            var projects = this.projectsCont.find(".grid-project"), visibleProjects = projects.filter(":not(.hidden)");
        ...
        highlightProject: function(num) {
            ...
            this.projectsCont.find(".grid-project").removeClass("current");
            if (num !== false) {
                // $(this.projectsCont.find('.grid-project')[this.selectedProject]).addClass('current');
                var visible = this.visibleProjects(), highlightMe = $(visible[this.selectedProject]), pos;
		 ...
		 */
        _events: function() {
            var that = this, eventType = "createTouch" in document ? "touchstart" : "click";
            ...
            $(window).on(eventType, "[data-get]", function(evt) {
                evt.preventDefault();
                var what = this.getAttribute("data-get"), hash = this.getAttribute("data-hash");
                that.get(what, hash);
            });
			...
			*/
            ...
            document.addEventListener("keydown", function(evt) {
                var keycode = evt.keyCode || false, current = that.current;
                if (keycode) {
                    var slug = false, dataContent = current.attr("data-content");
                    // escape
                    if (keycode === 27) {
...
(function(M) {
    ...
    String.prototype.queryToObj = function() {
        var obj = {}, queryParams = this.split("&"), i = 0, l = queryParams.length;
        for (i; i < l; i++) {
    ...
    // DOM caching
    var viewCont = $('[data-content="view"]'), detailCont = $('[data-content="detail"]'), projectsCont = $('[data-content="projects"]'), profileCont = $('[data-content="profile"]'), introCont = $('[data-content="intro"]'), tagsCont = $('[data-content="tags"]'), togglers = $("#togglers"), navLinks = $(".nav-link"), loader = $("#loader"), logo = $("#logo");
